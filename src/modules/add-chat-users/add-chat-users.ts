...
export class AddChatUsers extends Component {
                ...
                .subscribe(
                    (e: Event) => {
                        ...
                        const cancelButton = target.closest(
                            ".add-chat__selected-user-cancel-icon",
                        )
                        // Тогда удаляем из списка
                        if (cancelButton) {
                            const selectedUser = target.closest(
                                ".add-chat__selected-user-container",
                            ) as HTMLElement
    ...
    private _getChildComponentsFromUserList(users: User[]): ComponentChild<UsersListItem>[] {
        const usersList = users.map(
            (u: User, index: number): ComponentChild<UsersListItem> => {
                let disabled = false
